...
/*	{STM32F2_GPIO_PORT_E, STM32F2_GPIO_PIN_11},
	...
	{STM32F2_GPIO_PORT_E, STM32F2_GPIO_PIN_15},*///add by dq***************88 
...
int fsmc_nor_psram_init(u32 cs, u32 bcr, u32 btr, u32 bwtr)
{
	...
	rv = STM32_FSMC->cs[cs].bcr;
        ...
        STM32_FSMC->cs[cs].btr=0x00000000;
	STM32_FSMC->wt[cs].bwtr =0x00000000;
	STM32_FSMC->cs[cs].bcr=0x00000000;

/*	STM32_FSMC->cs[cs].bcr |=1<<12;
	STM32_FSMC->cs[cs].bcr |=0<<4;

	STM32_FSMC->cs[cs].btr |=8<<8;
	STM32_FSMC->cs[cs].btr |=0<<4;	
	STM32_FSMC->cs[cs].btr |=0<<0;	*/
	
//	STM32_FSMC->wt[cs].bwtr =0x0FFFFFFF;	
//	STM32_FSMC->wt[cs].bwtr |=1<<0;
...
#define CONFIG_SYS_FSMC_PSRAM_BWTR	0x0FFFFFFF*/
        ...
        STM32_FSMC->cs[cs].btr |= 0x00000800;
	
	STM32_FSMC->wt[cs].bwtr |= 0x0FFFFFFF;
	STM32_FSMC->cs[cs].bcr |= 0x00001001;
	rv = 0;
out:
	return rv;
}
